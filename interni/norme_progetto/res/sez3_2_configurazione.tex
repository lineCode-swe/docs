\subsection{Gestione della Configurazione}
    ...
    \item un'automazione, parte del sistema di Continuous Integration (o \glock{CI}), specifica per il prodotto contenuto nella repository, verrà eseguita automaticamente sul \glock{branch} principale per verificare che l'integrazione tramite \glock{merge} sia andata a buon fine con controlli automatici e report.
    \end{enumerate}
		...
		\begin{enumerate}
			\item in prossimità di una \glock{milestone}, l'amministratore crea un \glock{branch} di "release" a partire dal \glock{branch} principale;
			\item il software sul \glock{branch} di release non riceve funzionalità ulteriori ma solo bugfix che verranno integrati anche nel \glock{branch} principale;
