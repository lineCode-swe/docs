...
Di queste quattro, solo le prime tre saranno soggette a test in quanto il Generatore di ostacoli è una componente molto architetturalmente molto banale e usa-e-getta, creata al solo scopo di simulare la ricezione di dati dai sensori delle Unità.

Ulteriori scelte specifiche riguardo alla stesura dei test, saranno indicate nelle sezioni seguenti ove opportuno.

=== Test di sistema
I test di sistema coinvolgono tutte le componenti che devono essere accese e collegate fra di loro. Vengono scritti all'interno della UI in quanto unico punto d'ingresso al sistema da parte dell'utente.
...
* la componente UI non prevede computazioni che comprendano varie unità ma semplici passaggi di segnali;
